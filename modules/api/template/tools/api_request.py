...
def main():
 ...
 req=urllib.request.Request(x.url,data=x.data.encode() if x.data else None,headers=p.get('headers',{}),method=m);timeout=c.get('limits',{}).get('request_timeout_seconds',20);time.sleep(c.get('limits',{}).get('request_spacing_ms',500)/1000)
 try:res=urllib.request.urlopen(req,timeout=timeout);status=res.status;body=res.read()
 except urllib.error.HTTPError as e:status=e.code;body=e.read()
 LOG.mkdir(parents=True,exist_ok=True);stamp=str(time.time_ns());(LOG/f'{stamp}.body').write_bytes(body);(LOG/f'{stamp}.json').write_text(json.dumps({'method':m,'url':x.url,'status':status,'scope':r['base_url'],'profile':x.profile},indent=2));print('HTTP',status);print(body[:12000].decode(errors='replace'))
