...
def strings(path: Path) -> str:
    try:
        proc = subprocess.run(
            ["strings", "-a", "-n", "5", str(path)], cwd=ROOT, text=True, errors="replace",
...
def main() -> int:
    ...
    scanned = artifact_dirs_pruned = 0
    ...
    for current, dirnames, filenames in os.walk(base, followlinks=False):
        ...
        if current_rel.startswith("lib/modules/"):
            ...
            if len(parts) == 3 and re.search(r"\d+\.\d+", parts[2]):
                ...
                fingerprints[key] = {
                    "component": "linux-kernel", "version": version,
                    "path": rel(current_path), "evidence": "module-directory",
                }
            ...
            for component, pattern in PATTERNS:
                for match in pattern.finditer(text):
                    ...
                    fingerprints[key] = {
                        ...
                        "path": rel(path), "evidence": "static-string",
                    }

    result = sorted(fingerprints.values(), key=lambda item: (item["component"], item["version"], item["path"]))
    grouped: dict[tuple[str, str], list[dict]] = {}
    for item in result:
        grouped.setdefault((item["component"], item["version"]), []).append(item)

    OUT.write_text(json.dumps({
        "schema_version": 3, "scanned_named_files": scanned,
    ...
    lines = [
        ...
        f"raw fingerprint locations: {len(result)}",
        f"component/version groups: {len(grouped)}", "",
        "These are local version/research anchors, not vulnerability findings.", "",
    ]
    for (component, version), items in sorted(grouped.items()):
        examples = "; ".join(item["path"] for item in items[:3])
        more = f"; +{len(items)-3} more" if len(items) > 3 else ""
        lines.append(f"- {component} {version} occurrences={len(items)} :: {examples}{more}")
    TXT.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"[+] Firmware component fingerprints: {len(grouped)} component/version groups from {scanned} named files")
    return 0
