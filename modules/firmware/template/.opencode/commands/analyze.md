...
1. Read `[engagement]`, `[orchestration]`, `[firmware]`, `[analysis]`, and `[secrets]` first. Refuse if `engagement.authorized` is not true. Respect `orchestration.max_parallel_agents`; never exceed it. Treat `analysis.max_ghidra_slices_per_hypothesis` as the normal hard Ghidra iteration budget, defaulting to 3 only for older workspaces that lack the field.
...
9. Keep native deep dives convergent. For one binary/hypothesis, do not exceed `analysis.max_ghidra_slices_per_hypothesis` during the normal automated assessment. Stop and preserve `NEEDS VALIDATION` when additional work would mostly be vendor-specific archaeology, require broad whole-program reversing, or depend on runtime/topology/hardware/backend/cross-device evidence. Do not add target-specific toolkit logic merely to finish one firmware image.
