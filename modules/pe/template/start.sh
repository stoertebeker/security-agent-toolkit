...
cd "$(dirname "$0")"
umask 077
mkdir -p work/tmp work/cache work/ghidra/projects work/ghidra/slices reports/subagents reports/tool-output reports/research findings
...
[[ -n "$O" ]] || { echo "OpenCode missing. Run: ./toolkit install pe" >&2; exit 1; }
exec "$O" --agent pe-security
