...
def probe() -> dict:
    ...
    if emulator:
        ...
        accel_rc = proc.returncode
        accel_output = (proc.stdout or "").strip()
    kvm_usable = bool(emulator and accel_rc == 0)
    plan = runtime_plan(host_arch, abis, meta, dynamic)
    tooling_ready = bool(emulator and sdkmanager and avdmanager and adb_bin)
    notes: list[str] = []
    available = bool(plan.get("supported", False))
    acceleration = "unknown"

    if str(dynamic.get("backend", "auto")) == "none":
        available = False
        notes.append("dynamic.backend=none")
    if plan.get("reason"):
        notes.append(str(plan["reason"]))
    if available and tooling_ready:
        if kvm_usable:
            acceleration = "kvm"
        elif dynamic.get("allow_software_emulation", True):
            acceleration = "software"
            notes.append("KVM unavailable; x86_64 system image will use -accel off and may be dramatically slower")
        else:
            available = False
            notes.append("KVM unavailable and dynamic.allow_software_emulation=false")
    elif available and not tooling_ready:
        available = False
        notes.append("Android Emulator tooling is not installed; run ./toolkit install apk --with-optional")

    if virt_kind == "container" and not kvm.exists():
        notes.append(f"{virt_name} container has no /dev/kvm; host-side KVM device passthrough is required for acceleration")
    elif virt_kind == "container" and kvm.exists() and not os.access(kvm, os.R_OK | os.W_OK):
        notes.append(f"{virt_name} exposes /dev/kvm but current user lacks read/write access")
    elif virt_kind == "vm" and not kvm.exists():
        notes.append(f"{virt_name} VM has no /dev/kvm; nested virtualization is likely not exposed")
    elif virt_kind == "bare" and virt_flag and not kvm.exists():
        notes.append("CPU virtualization flags exist but /dev/kvm is absent")
    if emulator and accel_rc not in (None, 0):
        notes.append("emulator -accel-check did not report usable KVM")

    result = {
        "host_arch": host_arch,
        "virtualization_kind": virt_kind,
        "virtualization_name": virt_name,
        "cpu_virtualization_flag": virt_flag,
        "dev_kvm_exists": kvm.exists(),
        "dev_kvm_rw": kvm.exists() and os.access(kvm, os.R_OK | os.W_OK),
        "emulator_installed": bool(emulator),
        "sdkmanager_installed": bool(sdkmanager),
        "avdmanager_installed": bool(avdmanager),
        "adb_installed": bool(adb_bin),
        "tooling_ready": tooling_ready,
        "emulator_accel_check_rc": accel_rc,
        "emulator_accel_check": accel_output,
        "kvm_usable": kvm_usable,
        "package": meta.get("package"),
        "min_sdk": meta.get("min_sdk"),
        "target_sdk": meta.get("target_sdk"),
        "target_abis": abis,
        "selected_image_abi": plan.get("image_abi"),
        "selected_api_override": plan.get("api_override"),
        "runtime_abi_mode": plan.get("abi_mode"),
    ...
    lines = [
        "# APK dynamic capability probe",
        f"host architecture: {host_arch}",
        f"virtualization: {virt_kind}/{virt_name}",
        f"cpu virtualization flag: {'yes' if virt_flag else 'no'}",
        f"/dev/kvm: {'rw' if result['dev_kvm_rw'] else 'present-no-rw' if kvm.exists() else 'absent'}",
        f"tooling ready: {'yes' if tooling_ready else 'no'}",
        f"KVM usable by emulator: {'yes' if kvm_usable else 'no'}",
        f"target ABIs: {', '.join(abis) if abis else 'no native ABI requirement'}",
        f"runtime ABI mode: {result.get('runtime_abi_mode') or 'unavailable'}",
        f"selected image ABI: {result.get('selected_image_abi') or 'none'}",
        f"selected API override: {result.get('selected_api_override') if result.get('selected_api_override') is not None else 'none'}",
        f"selected acceleration: {acceleration}",
        f"dynamic available: {'yes' if available else 'no'}",
        "",
...
def sdk_available_packages() -> str:
    sdkmanager = which("sdkmanager")
    if not sdkmanager:
        fail("sdkmanager missing; run ./toolkit install apk --with-optional")
    proc = run([sdkmanager, "--sdk_root=" + str(SDK_ROOT), "--list"], timeout=180)
    if proc.returncode != 0:
        fail("sdkmanager --list failed")
    return proc.stdout or ""


def choose_system_image(cap: dict, dynamic: dict, meta: dict) -> tuple[str, int, str]:
    listing = sdk_available_packages()
    override = cap.get("selected_api_override")
    if override is not None:
        apis = [int(override)]
    else:
        requested = int(dynamic.get("api_level") or 0)
        if requested <= 0:
            requested = int(meta.get("target_sdk") or 36)
        min_sdk = int(meta.get("min_sdk") or 1)
        apis = list(range(requested, max(min_sdk, requested - 3) - 1, -1))
    configured_tag = str(dynamic.get("image_tag", "auto"))
    tags = ["default", "google_apis"] if configured_tag == "auto" else [configured_tag]
    abi = str(cap["selected_image_abi"])
    for api in apis:
        for tag in tags:
            package = f"system-images;android-{api};{tag};{abi}"
            if package in listing:
                return package, api, tag
...
def write_avd_config(avd_dir: Path, dynamic: dict) -> None:
    ...
    updates = {
        "hw.ramSize": str(int(dynamic.get("memory_mb", 4096))),
        "hw.cpu.ncore": str(int(dynamic.get("cores", 4))),
        "hw.keyboard": "yes",
        "showDeviceFrame": "no",
    ...
    lines = read_text(cfg).splitlines()
    seen: set[str] = set()
    output: list[str] = []
    for line in lines:
...
def setup() -> dict:
    mkdirs()
    _, dynamic = load_config(True)
    cap = probe()
    if not cap["dynamic_available"]:
        fail("dynamic capability probe reports UNAVAILABLE; see reports/tool-output/dynamic-capabilities.txt")
    meta = package_metadata()
    image_package, api, tag = choose_system_image(cap, dynamic, meta)
    ...
    if proc.returncode != 0:
        fail("system image installation failed; see reports/dynamic/sdkmanager-system-image.txt")

    project_slug = re.sub(r"[^A-Za-z0-9_-]+", "-", ROOT.name).strip("-")[:40] or "apk"
    abi_slug = str(cap["selected_image_abi"]).replace("-", "_")
    avd_name = f"sat-{project_slug}-api{api}-{abi_slug}"
    ...
    if proc.returncode != 0 or not avd_dir.exists():
        fail("AVD creation failed; see reports/dynamic/avd-create.txt")
    write_avd_config(avd_dir, dynamic)

    result = {
        "avd_name": avd_name,
        "avd_dir": str(avd_dir.relative_to(ROOT)),
        "system_image_package": image_package,
        "api_level": api,
        "image_tag": tag,
        "image_abi": cap["selected_image_abi"],
        "runtime_abi_mode": cap["runtime_abi_mode"],
        "acceleration": cap["selected_acceleration"],
        "root_expected": True,
        "package": meta.get("package"),
        "target_abis": cap["target_abis"],
        "virtualization_kind": cap["virtualization_kind"],
        "virtualization_name": cap["virtualization_name"],
    }
...
def adb_serial(dynamic: dict) -> str:
    return f"emulator-{int(dynamic.get('emulator_port', 5554))}"
...
def device_info(dynamic: dict) -> dict:
    props: dict[str, str] = {}
    proc = adb(dynamic, "shell", "getprop", timeout=30)
...
def verify_runtime_abi(info: dict) -> None:
    ...
    compatible = sorted(supported.intersection(required))
    ...
    if not compatible:
        fail(f"emulator booted but exposes none of the package native ABIs {required}; see reports/dynamic/abi-compatibility.json")
...
def capture_state(dynamic: dict, label: str) -> None:
    ...
    for filename, args in commands.items():
        (outdir / filename).write_text(adb(dynamic, *args, timeout=60).stdout or "", encoding="utf-8")
    if "uid=0(" in (adb(dynamic, "shell", "id", timeout=30).stdout or ""):
        ...
        if pids:
            (outdir / "process-maps.txt").write_text(adb(dynamic, "shell", "cat", f"/proc/{pids[0]}/maps", timeout=30).stdout or "", encoding="utf-8")


def start() -> None:
    mkdirs()
    _, dynamic = load_config(True)
    setup_data = read_setup()
    if pid_alive(EMULATOR_PID):
        print("[=] Emulator already running")
        return
    emulator = which("emulator")
    if not emulator:
        fail("emulator missing; run ./toolkit install apk --with-optional")
    ...
    cmd = [
        ...
        "-no-snapshot",
        "-gpu", "software",
        "-accel", "on" if setup_data["acceleration"] == "kvm" else "off",
        "-tcpdump", str(REPORT / "network.pcap"),
    ]
    if dynamic.get("headless", True):
        cmd.append("-no-window")
    if dynamic.get("wipe_data_on_start", True):
        cmd.append("-wipe-data")
    handle = EMULATOR_LOG.open("ab")
    proc = subprocess.Popen(cmd, cwd=ROOT, env=env(), stdout=handle, stderr=subprocess.STDOUT, start_new_session=True)
    EMULATOR_PID.write_text(str(proc.pid) + "\n", encoding="utf-8")
    print(f"[*] Emulator PID {proc.pid}; waiting for boot ({setup_data['acceleration']})", flush=True)

    serial = adb_serial(dynamic)
    adb_bin = which("adb") or "adb"
    timeout_seconds = int(dynamic.get("boot_timeout_seconds", 600))
    deadline = time.monotonic() + timeout_seconds
    run([adb_bin, "-s", serial, "wait-for-device"], timeout=timeout_seconds)
    while time.monotonic() < deadline:
        ...
        if proc.poll() is not None:
            fail("emulator exited during boot; see reports/dynamic/emulator.log")
        time.sleep(3)
    else:
    ...
    if dynamic.get("request_root", True):
        root_proc = adb(dynamic, "root", timeout=60)
        root_result["output"] = (root_proc.stdout or "").strip()
        time.sleep(2)
        run([adb_bin, "-s", serial, "wait-for-device"], timeout=60)
    ...
    info = device_info(dynamic)
    verify_runtime_abi(info)
    start_logcat(dynamic)
    capture_state(dynamic, "preinstall")
...
def install_app() -> None:
    mkdirs()
    cfg, dynamic = load_config(True)
    start()
    package = package_name()
    input_path = (ROOT / cfg["apk"]["path"]).resolve()
    if input_path.suffix.lower() == ".xapk":
        inventory = TOOL_REPORT / "xapk-inventory.json"
        if not inventory.is_file():
            fail("XAPK inventory missing; run tools/apk_prepare.py first")
        data = json.loads(inventory.read_text())
        apks = [ROOT / data["base_apk"]] + [ROOT / item["file"] for item in data.get("splits", [])]
    ...
    proc = adb(dynamic, *args, timeout=300)
    (REPORT / "install.txt").write_text(proc.stdout or "", encoding="utf-8")
    if proc.returncode != 0 or "Success" not in (proc.stdout or ""):
        fail("APK installation failed; see reports/dynamic/install.txt")
    (REPORT / "installed-package-paths.txt").write_text(adb(dynamic, "shell", "pm", "path", package, timeout=60).stdout or "", encoding="utf-8")
    capture_state(dynamic, "postinstall")
...
def screenshot(dynamic: dict) -> None:
    ...
    if proc.returncode == 0 and isinstance(proc.stdout, bytes):
        (REPORT / "screenshot.png").write_bytes(proc.stdout)
...
def launch() -> None:
    ...
    start()
    package = package_name()
    adb(dynamic, "logcat", "-c", timeout=30)
    ...
    time.sleep(int(dynamic.get("observation_seconds", 15)))
    ui_dump(dynamic)
    screenshot(dynamic)
    (REPORT / "launch-logcat.txt").write_text(adb(dynamic, "logcat", "-d", "-v", "threadtime", timeout=60).stdout or "", encoding="utf-8")
    capture_state(dynamic, "postlaunch")
    print(f"[+] Launched and observed {package}")


def ensure_frida_server(dynamic: dict) -> None:
    if not dynamic.get("allow_frida", False):
        fail("dynamic.allow_frida=false")
    if not which("frida") or not which("frida-ps"):
        fail("Frida host tools missing")
    root = json.loads((REPORT / "root-status.json").read_text()) if (REPORT / "root-status.json").is_file() else {}
    if not root.get("available"):
        fail("Frida injected mode requires adb-root/rooted emulator in this toolkit flow")
    version = (run([which("frida") or "frida", "--version"], timeout=30).stdout or "").strip().splitlines()[0]
    info = device_info(dynamic)
    abi = info.get("abi") or ""
    frida_arch = {"arm64-v8a": "arm64", "armeabi-v7a": "arm", "x86_64": "x86_64", "x86": "x86"}.get(abi)
    if not frida_arch:
        fail(f"unsupported Frida server ABI: {abi}")
    cache = SAT_HOME / "cache"
    tools = SAT_HOME / "tools" / "frida-server"
    cache.mkdir(parents=True, exist_ok=True)
    tools.mkdir(parents=True, exist_ok=True)
    packed = cache / f"frida-server-{version}-android-{frida_arch}.xz"
    binary = tools / f"frida-server-{version}-android-{frida_arch}"
    if not binary.is_file():
        url = f"https://github.com/frida/frida/releases/download/{version}/frida-server-{version}-android-{frida_arch}.xz"
        print(f"[*] Downloading Frida server {version} for {frida_arch}", flush=True)
        urllib.request.urlretrieve(url, packed)
        binary.write_bytes(lzma.decompress(packed.read_bytes()))
        binary.chmod(0o755)
    remote = "/data/local/tmp/sat-fs"
    adb(dynamic, "push", str(binary), remote, timeout=180)
    adb(dynamic, "shell", "chmod", "755", remote, timeout=30)
    adb(dynamic, "shell", "sh", "-c", f"pkill -f '{remote}' >/dev/null 2>&1 || true", timeout=30)
    adb(dynamic, "shell", "sh", "-c", f"{remote} >/data/local/tmp/sat-frida.log 2>&1 &", timeout=30)
    time.sleep(2)
    smoke = run([which("frida-ps") or "frida-ps", "-U"], timeout=60)
    (REPORT / "frida-smoke.txt").write_text(smoke.stdout or "", encoding="utf-8")
    if smoke.returncode != 0:
        fail("frida-ps -U failed; see reports/dynamic/frida-smoke.txt")


def instrument_start() -> None:
    _, dynamic = load_config(True)
    start()
    ensure_frida_server(dynamic)
    script = ROOT / "tools" / "frida_observe.js"
    if not script.is_file():
        fail("tools/frida_observe.js missing")
    output = REPORT / "frida-events.txt"
    cmd = [which("frida") or "frida", "-U", "-f", package_name(), "-l", str(script), "-q", "-t", "inf", "-o", str(output)]
    proc = subprocess.Popen(cmd, cwd=ROOT, env=env(), stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT, start_new_session=True)
    (WORK / "frida.pid").write_text(str(proc.pid) + "\n", encoding="utf-8")
...
def instrument_stop() -> None:
    ...
    if pid_alive(pidfile):
        os.killpg(int(pidfile.read_text().strip()), signal.SIGTERM)
        print("[+] Frida observation stopped")
...
def collect() -> None:
    ...
    ui_dump(dynamic)
    screenshot(dynamic)
    (REPORT / "logcat-final.txt").write_text(adb(dynamic, "logcat", "-d", "-v", "threadtime", timeout=60).stdout or "", encoding="utf-8")
    capture_state(dynamic, "final")
    summary = {
        "package": package_name(),
        "network_capture": "reports/dynamic/network.pcap",
        "logcat": "reports/dynamic/logcat-final.txt",
        "ui_dump": "reports/dynamic/ui.xml",
        "screenshot": "reports/dynamic/screenshot.png",
        "frida_events": "reports/dynamic/frida-events.txt" if (REPORT / "frida-events.txt").exists() else None,
    }
...
def status() -> None:
    cap = probe()
    setup_data = json.loads(SETUP_JSON.read_text()) if SETUP_JSON.is_file() else None
    print(json.dumps({
        "capabilities": cap,
        "setup": setup_data,
        "emulator_running": pid_alive(EMULATOR_PID),
        "logcat_running": pid_alive(LOGCAT_PID),
        "frida_running": pid_alive(WORK / "frida.pid"),
    }, indent=2))
...
def main() -> int:
    ...
    if action == "probe":
        ...
        collect()
    elif action == "frida-start":
        instrument_start()
    elif action == "frida-stop":
        instrument_stop()
    elif action == "status":
        status()
    elif action == "stop":
